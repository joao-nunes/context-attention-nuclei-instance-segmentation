...
class RoIHeads(nn.Module):
    ...
    def __init__(
        ...
        self.batch_size_per_img = batch_size_per_image
    ...
    def forward(
        ...
        box_features_ = self.box_head(box_features)
        
        for _ in range(self.relation_steps):
            class_logits, box_regression = self.box_predictor(box_features_)
            scores = F.softmax(class_logits, dim=-1)
            pred_boxes = self.box_coder.decode(box_regression, proposals)
            pred_boxes = box_ops.clip_boxes_to_image(pred_boxes, image_shapes[0])
            indices = torch.argmax(scores, dim=-1)
            pred_boxes = torch.stack([pred_boxes[i,indices[i],...] for i in range(pred_boxes.shape[0])], 0)
            box_features_ = self.box_head(box_features, pred_boxes)
