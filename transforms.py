...
class TargetFormatting():
    def __init__(self, target_dict: Dict[str, Tensor]):
        self.target_dict = target_dict

    def __call__(self, image: Tensor, target: Tensor) -> Tuple[Tensor, Tensor]:
        return image, self.target_dict['target']
    ...
    def forward(
        ...
        if torch.rand(1) < self.p:
            ...
            if target is not None:
                ...
                width, _ = torchvision.transforms.functional.get_image_size(
                    image)
...
class RandomIoUCrop(nn.Module):
    def __init__(
        ...
        # https://github.com/weiliu89/caffe/blob/ssd/examples/ssd/ssd_coco.py#L89-L174
    ...
    def forward(
        ...
        if isinstance(image, torch.Tensor):
            if image.ndimension() not in {2, 3}:
                raise ValueError("image should be 2/3 dimensional. "
                                 f"Got {image.ndimension()} dimension.")
...
class RandomZoomOut(nn.Module):
    def __init__(
        self, fill: Optional[List[float]] = None,
        ...
        if side_range[0] < 1.0 or side_range[0] > side_range[1]:
            raise ValueError("Invalid canvas side "
                             f"range provided {side_range}.")
        self.p = p
