...
from se_resnet import ResNetBottleneck
...
def parse_args():
    ...
    parser.add_argument(
        "--out", type=str,
        default="mask-rcnn-se-resnet50-fpn-dpath-fixed"
    )

    parser.add_argument("--source", type=str, default="dpath")
    parser.add_argument("--bottleneck", type=str, default="ResNetBottleneck")
    parser.set_defaults(
...
def get_model_instance_segmentation(bottleneck, num_classes, use_srm):
    # load an instance segmentation model pre-trained on COCO
    model = torchvision.models.detection.maskrcnn_resnet50_fpn(weights=MaskRCNN_ResNet50_FPN_Weights.DEFAULT)
    state_dict = model.backbone.body.state_dict()
    if bottleneck == "SEBottleneck":
        pretrained = "https://github.com/moskomule/senet.pytorch/releases/download/archive/seresnet50-60a8950a85b2b.pkl"
    else:
        pretrained = None
    model.backbone.body = ResNetExt.resnet50(
            3,
...
class ResNetExt(ResNet):
    ...
    def _forward_impl(self, x, freeze):
        ...
        if self.use_srm:
            shape = x4.shape
            h = torch.flatten(x4, start_dim=-2)
            h = torch.swapaxes(h, -1, -2) # batch, seq, feature
            z = self.srmodule(h)
            x4 = torch.reshape(torch.swapaxes(z, -1, -2), shape)
    ...
    def resnet50(
        ...
        if bottleneck == "ResNetBottleneck":
            ...
            if use_srm:
                ...
                srmodule = torch.nn.TransformerEncoder(
                    encoder_layer,
                    num_layers=1
                    )
                setattr(model, "srmodule", srmodule)
            else:
                setattr(model, "use_srm", False)
        ...
        model.bn1 = FrozenBatchNorm2d(64, eps=0.0)
...
def main(
    ...
    model = get_model_instance_segmentation(bottleneck, num_classes, use_srm)
